...
export const getAllMoves = (
  ...
  col,
  currentPlayer,
  cells = this.state.gameState
) => {
  let moves = [];
  if (this.state.gameState[row][col] === 0) {
    moves = moves.concat(checkVerticalTop(row, col, currentPlayer, cells));
    moves = moves.concat(checkVerticalBottom(row, col, currentPlayer, cells));
    moves = moves.concat(checkHorizontalRight(row, col, currentPlayer, cells));
    moves = moves.concat(checkHorizontalLeft(row, col, currentPlayer, cells));
    moves = moves.concat(checkDiagonalTopLeft(row, col, currentPlayer, cells));
    moves = moves.concat(checkDiagonalTopRight(row, col, currentPlayer, cells));
    moves = moves.concat(
      checkDiagonalBottomLeft(row, col, currentPlayer, cells)
    );
    moves = moves.concat(
      checkDiagonalBottomRight(row, col, currentPlayer, cells)
    );
  }
...
// // Helpers
// export const getCellPosition = pos => {
//   let row = parseInt(pos / SIZE);
//   let col = pos % SIZE;
//   return { row, col };
// };

// export const opposite = status => {
//   if (CELL_STATUS.BLACK === status) return CELL_STATUS.WHITE;
//   else return CELL_STATUS.BLACK;
// };

let cells = this.state.gameState;

// Vertical
export const checkVerticalTop = (row, col, status, cells) => {
  let max = -1;
  let min = row;
...
export const checkVerticalBottom = (row, col, status, cells) => {
  let max = row;
  let min = -1;
...
export const calculateColumn = (min, max, col, currentPlayer, cells) => {
  let moves = [];
  if (min > -1 && max > -1) {
...
// Horizontal
export const checkHorizontalRight = (row, col, status, cells) => {
  let max = -1;
  let min = col;
...
export const checkHorizontalLeft = (row, col, status, cells) => {
  let max = col;
  let min = -1;
...
export const calculateRow = (min, max, row, status, cells) => {
  let moves = [];
  if (min > -1 && max > -1) {
    for (let i = min; i < max; i++) {
...
export const calculateDiagonal = (
  num,
  row,
  col,
  rowOp,
